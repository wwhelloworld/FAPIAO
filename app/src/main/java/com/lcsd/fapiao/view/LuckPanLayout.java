...
public class LuckPanLayout extends RelativeLayout {
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        MinValue -= Mytools.dip2px(context, 10) * 2;
    ...
    private void drawSmallCircle(boolean FirstYellow) {
        int pointDistance = radius - Mytools.dip2px(context, 10);
        for (int i = 0; i <= 360; i += 20) {
            int x = (int) (pointDistance * Math.sin(Mytools.change(i))) + CircleX;
            int y = (int) (pointDistance * Math.cos(Mytools.change(i))) + CircleY;

            if (FirstYellow)
                canvas.drawCircle(x, y, Mytools.dip2px(context, 4), yellowPaint);
            else
                canvas.drawCircle(x, y, Mytools.dip2px(context, 4), whitePaint);
